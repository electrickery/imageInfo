...
/* Assumes the sectors are in numerical order which normally is the case for Epson disks
unsigned long int getSectorLocation(int track, int record, int driveId) {
    unsigned long location = SECTORBASE +
            track * TRACK * HEADS +
            (record - 1) * (RECORD) +
            SECTORHEADER + SECTORHEADER * ((record - 1)  / (SECTOR / RECORD));
    msg(LOG_DEBUG, "   getSectorLocation; Track: %02X Record: %02X => location: %06lX\n",
